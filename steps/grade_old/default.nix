...
let
  autograde = pkgs.dhallPackages.buildDhallPackage {
    name = "autograde";
    dependencies =[ pkgs.dhallPackages.Prelude ];
    code = ./autograde.dhall;
    source = true;
  };
in
pkgs.runCommandLocal "grade" {
  ...
  echo "${autograde}/source.dhall (./comments.txt as Text) $warnings/warnings.dhall" | dhall text > $comments/comments.txt
